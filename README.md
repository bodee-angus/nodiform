...
The default experiment is a small eight-node chain with optional colour inputs. **Experiment → Examples** contains **Connect to every earlier node**, **Connect to the previous half**, **Letter permutations**, **Growing ring**, **Ring lattice**, **Modular residues**, **Prime factors**, **Digits of pi**, **Divisor graph**, **Toroidal grid**, **Grid**, and **Fibonacci chain**. The complete-growth and previous-half examples default to 500 nodes: connecting to every earlier node creates 124,750 edges, while connecting to the previous half creates 62,500. These are editable experiments, not fixed application modes.

**Ring lattice** creates nodes 1 through **Node Count**, each connected to **Previous Node Connect Count** preceding nodes in a cyclic sequence. The defaults of 10 and 2 produce 20 unique undirected edges; node 1 eventually has neighbours 2, 3, 9 and 10. Wraparound edges appear when both endpoints exist. Zero predecessors gives isolated nodes; overlapping neighbour selections never add duplicate edges or self-loops. Positions emerge from the force simulation rather than being placed on a circle.
...
Every example exposes **Colour by** in Inputs. Birth-order modes advance through a full rainbow; structural modes give each newly encountered category the next palette colour. Repeated categories keep their colour.

Generated colours share one OKLCH lightness and chroma; only hue varies. Palettes are sorted by hue, starting at red. Spacing depends on the requested count, so increasing the count changes earlier colours. Fixed equal lightness and chroma limit how saturated all hues can be together; the palette uses nearly the maximum common sRGB chroma. Small deviations remain after conversion to 8-bit hex. Saved script source is preserved, but rerunning a script that calls `palette` uses the new colours. Literal hex colours are unchanged.
