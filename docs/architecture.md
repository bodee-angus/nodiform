...
The deterministic hue sequence distributes successive samples around the circle for category separation. Increasing a count preserves the current version's colour prefix, returned arrays are independent, and palette generation does not draw from the experiment's random stream. The fixed-lightness/chroma circle has finitely many 8-bit hex representations; sufficiently large palettes can repeat colours and cannot keep all categories visually distinct. Counts must be nonnegative safe integers and the returned array must fit available memory and JavaScript array representation limits.

Version 0.1.7 replaces the earlier variable-lightness/chroma palette and identifies the change with `nodiform-rules-v5`. Existing source is preserved, but any rerun that calls the helper uses the new palette; literal hex values are unaffected. The force and birth-policy versions do not change. Toroidal-grid dimensions are script-level connectivity coordinates only, so adding that example does not introduce higher-dimensional physics or rendering.
