...
**Ring lattice** creates nodes `1` through **Node Count** `n`. Each node selects the **Previous Node Connect Count** `k` preceding nodes around the cyclic numbering, so the predecessors of `1` include `n`, `n − 1`, and so on. Every connection is undirected and appears only once, with the later endpoint's birth. With the defaults `n = 10, k = 2`, node 9 adds its wraparound edge to 1 and node 10 adds its wraparound edges to 1 and 2. There are 20 edges in total, and every node has four neighbours. In general, each node has `min(2k, n − 1)` neighbours and the graph has `n × min(2k, n − 1) / 2` edges. Zero predecessors creates isolated nodes; a single node has no edges. Counts larger than the available neighbours add no self-loops or duplicate attraction. There is no fixed node-count ceiling. **Colour by** supports birth-order and ring-distance: the latter shares colours by the shortest distance around the numbered ring from node 1, regardless of the chosen connection count. Both allocate successive palette hues. Connection strength, birth timing and final settling remain editable. No circular positions are prescribed. Source: [ring-lattice.js](../examples/ring-lattice.js).
