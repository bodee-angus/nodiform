...
// Hubs and their occurrences share a colour; sequence edges use gradients.
...
function* generate(N, params) {
    ...
    const colors = N.palette(10);
    const occurrences = Array(10).fill(0);

    // Hubs have no connections until their digit appears.
    yield N.batch(
        colors.map((color, digit) => N.node(String(digit), {
            label: String(digit), color, radius: 2.4
        })),
        []
    );
    ...
    for (let index = 0; index < count; index++) {
        ...
        yield N.batch(
            [N.node(id, { label: id, color: colors[digit], radius: 1.6 })],
            edges
        );
        previous = id;
        yield N.wait(ticks);
