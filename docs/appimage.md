...
Nodiform 0.1.9 is an **experimental alpha** for x86_64 Linux. The AppImage gives it a self-contained application file, desktop-menu entry, and a Gear Lever update source. Experiments and recordings stay outside the application file. This packaging does not sandbox Nodiform or change its alpha status.
...
Version 0.1.9 uses the same update channel, so an existing Gear Lever installation needs no new source. It adds **Grid** under **Experiment → Examples**, a minimum visible edge width when zoomed out, and approximate time remaining on the preview and recording progress bar. At the default thickness, edges stop shrinking at one physical pixel; the existing thickness slider scales that minimum. Saved script source is preserved.

Generated palettes now follow rainbow hue order at fixed OKLCH lightness and chroma, using nearly the strongest common sRGB chroma. Requesting a different count changes colour spacing and earlier entries. Scripts using `palette` receive these new colours when rerun, including saved scripts; literal hex colours remain unchanged. Graph storage still grows without the former 8,192-node and 250,000-edge caps removed in 0.1.5. The force model and automatic birth placement from 0.1.4 are unchanged. Available memory and the GPU’s actual limits still apply; the exact solver can become very slow as node counts grow.
