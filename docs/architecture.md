...
Gradient edges sample the live node-style buffer at both endpoints and interpolate linear-light RGB and alpha along their length. The edge colour's alpha multiplies the endpoint alpha; its RGB is ignored in gradient mode. A later node colour change therefore updates connected gradients without rewriting edge events. Solid colour remains the default for older edges. Preview and recording use the same render pipeline. A project-level edge-thickness multiplier scales the world-space edge width from its default 0.18. The slider range is 0.25×–8×; zoom still scales the displayed width. It is independent of edge strength and is captured in recording metadata. Old projects default to 1×. Preview can change it live; recording locks it to the captured value.
